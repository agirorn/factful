...
exports.config = {
   ...
   * path portion of your baseUrl.  If your `url` parameter starts without
   * a scheme or `/` (like `some/path`), the base url gets prepended directly.
   */
   ...
   * @param  {[type]} caps     object containing capabilities for session that
   *                           will be spawn in the worker
   * @param  {[type]} specs    specs to be run in the worker process
   * @param  {[type]} args     object that will be merged with the main
   *                           configuration once worker is initialised
   * @param  {[type]} execArgv list of string arguments passed to the worker
   *                           process
   */
